...
import 'package:path/path.dart'
    show Context; // flutter_ignore: package_path_import -- We only use Context as an interface.
...
void main() {
  group('installation detection on MacOS', () {
    ...
    testUsingContext('discovers explicitly configured Android Studio', () {
      final String extractedDownloadZip = fileSystem.path.join('/', 'Users', 'Dash', 'Desktop', 'android-studio');
  ...
  group('latestValid', () {
    ...
    testUsingContext('choses the install with the latest version', () {
      ...
      for (final String version in versions) {
        ...
        fileSystem.directory('C:\\Program Files\\AndroidStudio$version').createSync(recursive: true);
      }
    ...
    testUsingContext('throws a ToolExit if --android-studio-dir is configured but the directory does not exist',
        () async {
      const String configuredAndroidStudioDir = r'C:\Users\Dash\Desktop\android-studio';
      config.setValue('android-studio-dir', configuredAndroidStudioDir);
